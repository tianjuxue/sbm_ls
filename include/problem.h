...
void NonlinearProblem<dim>::setup_system()
{
       ...
       cell != dof_handler.end(); ++cell)
  {
    ...
    for (unsigned int v = 0; v < GeometryInfo<dim>::vertices_per_cell; ++v)
    {
      ...
      counter++;
      bool is_vertex_inside = is_inside(dof_handler_all, solution_all, cell->vertex(v));
    ...
    if (cell->active_fe_index() == 1)
    {
      ...
      for (unsigned int q = 0; q < n_q_points; ++q)
      {
        ...
        Tensor<2, dim> A_tensor = (1. - 1. / grad_norm) * unit_tensor +
                                  outer_product(solution_gradients[q], solution_gradients[q]) /
                                  pow(grad_norm, 3.);
      ...
      {
        // Only interface boundary is considered as boundary
        bool is_on_boundary;
        if (cell->at_boundary())
          is_on_boundary = false;
        else
          is_on_boundary = cell->neighbor(face_no)->active_fe_index() == 0;
        ...
        if (is_on_boundary)
        {
          ...
          sbm_map(target_points, normal_vectors, distance_vectors, fe_values_face.get_quadrature_points(), n_face_q_points, dof_handler_all, solution_all);
          ...
          for (unsigned int q = 0; q < n_face_q_points; ++q)
          {
            ...
            for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
              ...
              local_rhs(i) += a_scalar_face * fe_values_face.normal_vector(q) *
